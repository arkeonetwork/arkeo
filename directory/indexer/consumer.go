...
func (s *Service) handleAbciEvent(event abcitypes.Event, transaction tmtypes.Tx, height int64) error {
	...
	case "create_validator", "edit_validator", "redelegate", "unbond", "complete_redelegation", "complete_unbonding":
		attrJSON, err := json.Marshal(event.Attributes)
		if err != nil {
			return err
		}
	...
	// Burn events
	case "burn", "slash", "jail", "unjail", "cosmos.authz.v1beta1.EventGrant", "cosmos.authz.v1beta1.EventRevoke":
		attrJSON, err := json.Marshal(event.Attributes)
		if err != nil {
			return err
		}
