...
var sqlGetNetworkStats = `
	SELECT 
	  total_contracts,
	  open_contracts,
	  COALESCE(median_open_contract_length, 0) AS median_open_contract_length,
	  COALESCE(median_open_contract_rate, 0) AS median_open_contract_rate,
	  total_online_providers,
...
var sqlGetNetworkStatsByService = `
	...
	(
		SELECT COALESCE(
			percentile_cont((0.5)::double precision) 
				WITHIN GROUP (ORDER BY ((c3.duration)::double precision)),
			0
		) AS percentile_cont
		FROM contracts c3
	...
	(
		SELECT COALESCE(
			percentile_cont((0.5)::double precision) 
				WITHIN GROUP (ORDER BY ((c4.rate_amount)::double precision)),
			0
		) AS percentile_cont
		FROM contracts c4
	...
	(
		SELECT COALESCE(sum(cse1.nonce), 0) AS coalesce
	...
	(
		SELECT COALESCE(sum(cse2.paid), 0) AS coalesce
