...
func (am AppModule) RegisterServices(cfg module.Configurator) {
	...
	// Migrations: no-op from v1 -> v2 (dynamic service registry intro)
...
// ConsensusVersion is a sequence number for state-breaking changes. Bumped to 2 for the dynamic service registry migration.
...
func (am AppModule) EndBlock(ctx context.Context) ([]abci.ValidatorUpdate, error) {
	mgr := keeper.NewManager(am.keeper, am.stakingKeeper)
	context := sdk.UnwrapSDKContext(ctx)
	if err := mgr.EndBlock(context); err != nil {
		context.Logger().Error("manager endblock error ", "error", err)
	}
